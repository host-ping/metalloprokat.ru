...
/**
 * @method Manufacturer getSubject()
 */
class ManufacturerAdmin extends AbstractAdmin
{
    ...
    {
        return $object instanceof Manufacturer ? 'Производитель '.$object->getAttributeValue()->getValue() : '';
    }
            ...
            ->add(
                ...
                array(
                    ...
                    'callback' => function ($queryBuilder, $alias, $field, $value) {
                        ...
                        if ($value['value'] === 'y') {
                            $queryBuilder->andWhere(sprintf('(EXISTS (SELECT p.id FROM MetalCatalogBundle:Product p WHERE p.manufacturer = %s.id))', $alias));
                        } else {
                            $queryBuilder->andWhere(sprintf('(NOT EXISTS (SELECT p.id FROM MetalCatalogBundle:Product p WHERE p.manufacturer = %s.id))', $alias));
                        }
        ...
        $formMapper
            ...
            ->add('uploadedLogo', VichImageType::class,
                array(
                    ...
                    'required' => !$manufacturer->getLogo()->getName(),
