...
//        $('u-menu:visible').parents('.login-user').addClass('active');
        ...
        $('.js-datepicker').each(function(i, el) {
            ...
            $el.data('datepicker.selectedDate', config.selectedDate ? moment(config.selectedDate) : moment());
        });
...
function initializeDatepicker($context) {
    $context = $context || null;
    $('.js-datepicker', $context).each(function (i, el) {
        // pre-initialize some values
        var $el = $(el);
        var config = $el.data('datepicker-config');
    ...
    $('.js-datepicker', $context).each(function (i, el) {
        ...
        var clndr = $el.clndr({
            daysOfTheWeek: getDaysOfWeek(),
            startWithMonth: $el.data('datepicker.selectedDate') ? $el.data('datepicker.selectedDate') : moment(),
            render: render,
            clickEvents: {
                click: function (target) {
                    var selectedDate = target.date;
                    ...
                    $el.trigger($.Event('datepicker.selectDate', {$datepicker: $el}));
                    clndr.render();
                }
            }
        });
...
function number_format_current_locale(number, decimals) {
